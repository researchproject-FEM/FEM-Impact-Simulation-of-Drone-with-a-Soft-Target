...
if __name__ == "__main__":
    ...
    hp = HeadNeckParams()
    sp = SimParams()
    results = simulate(bp, hp, sp, do_animation=True)
    if results["fractures"]:
        print("Fracture events:")
        for st, elems in results["fractures"]:
            print(f"  step={st}, t={st*results['dt']:.6f}s, broken={elems}")
    else:
        print("No fractures detected.")
    print_injury_report(results, hp)
    plot_head_acceleration(results, hp)
    plot_plasticity(results, bp)
